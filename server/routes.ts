...
// Video Analysis routes - using Gemini AI
export const videoAnalysisRoutes = {
  ...
  async getQuickFeedback(videoPath: string) {
    try {
      const { getQuickFeedback } = await import('./gemini-video-analysis');
      return await getQuickFeedback(videoPath);
    } catch (error) {
      throw new Error(`Failed to get quick feedback: ${error}`);
    }
  },

  async compareVideoSegments(videoPaths: string[], description: string) {
    try {
      const { compareVideoSegments } = await import('./gemini-video-analysis');
      return await compareVideoSegments(videoPaths, description);
    } catch (error) {
      throw new Error(`Failed to compare videos: ${error}`);
    }
  },
};
