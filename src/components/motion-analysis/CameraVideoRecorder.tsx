...
const QUALITY_SETTINGS: Record<VideoQuality, { width: number; height: number; frameRate: number; videoBitrate: number }> = {
...
const CameraVideoRecorder = ({ onBack, onComplete }: CameraVideoRecorderProps) => {
  ...
  }, [getSupportedMimeType, startCamera, stopCamera, toast]);
  ...
  }, [videoQuality, isRecording, isPreviewing, hasPermission, startCamera, stopCamera]);
  ...
  const startCamera = useCallback(async () => {
    const settings = QUALITY_SETTINGS[videoQuality];
            ...
            {!isPreviewing && hasPermission && (
              <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center gap-2 bg-black/60 px-3 py-1.5 rounded-full">
                <span className="text-white text-xs">
                  {videoQuality} • {QUALITY_SETTINGS[videoQuality].frameRate}fps
                </span>
              </div>
            )}
          </div>
